...
export function conditionsForChild(data: Data, child: FormChildren) {
  return (child.conditions ?? child.suppressIf ?? [])
    .map((offset) => data.suppressions.find((condition) => condition.offset === offset))
    .filter((condition): condition is Suppression => condition !== undefined);
}

export function childVisibility(data: Data, child: FormChildren): VisibilityInfo {
  ...
  if (suppressions.length > 0) {
    const alwaysHidden = suppressions.some((condition) => condition.constant === true);
    return {
      status: "hidden",
      gate: "suppression",
...
export function combineVisibility(
  ...
  const childGate = child === "hidden" || child === "conditional" ? child : "visible";
...
function findReferencedForm(data: Data, formId: string, formSetGuid?: string) {
  const normalized = normalizedFormId(formId);
  const inFormSet = data.forms.findIndex(
    (form) =>
      (form.formSetGuid ?? "").toLowerCase() === (formSetGuid ?? "").toLowerCase() &&
      normalizedFormId(form.formId) === normalized,
  );

  return inFormSet >= 0
    ? inFormSet
    : data.forms.findIndex((form) => normalizedFormId(form.formId) === normalized);
}
...
export function summarizeFormBranch(
  ...
  function visit(
    ...
    for (const child of form.children) {
      ...
      let status = combineVisibility(inheritedStatus, visibility.status);
      let targetIndex = -1;
      ...
      if (child.type === "Ref" && targetIndex >= 0 && !nextAncestors.has(targetIndex)) {
